...
root_in = "E:/VIA/scan_IPhone/mbalmayo001/" #path to the main folder of the lidar scans files
...
Import all the the lidar scan files within a list (list_of_Lidar). Each element corresponf to one file
with 3 variables (id, X_lidar, Y_lidar)

files_summary resumes the informations included in each files
...
```{r}
dir.create(paste0(plot_name,"/20_20_PROJECTION"))

{
   ...
   for (i in 1:length(unique(all$file))) {
      ...
      jalon_ref <- jalon_ref[jalon_ref$jalon %in% tmp$id,]
   ...
   for (i in 1:length(unique(all$sousplot))){
      ...
      png(paste0(plot_name,"/20_20_PROJECTION/",'20_20_PROJECTION_', str_remove(unique(tmp$file),'.csv'),'.png'), width = 600)
      print(my_plot)
      dev.off()
   }
} # REPROJECT TREES TO 20 20 coordinates

dir.create(paste0(plot_name,"/Field_scan_comparison"))

{
   ...
   all_comparison <- all %>% filter((!(is.na(position_x)) & !(is.na(position_y))) | what == 'jalon') %>% filter (sousplot == sous_plot_name)
   ...
   for (i in 1:length(unique(all_comparison$sousplot))) {
      ...
      png(paste0(plot_name,"/Field_scan_comparison/",'Field_scan_comparison_', str_remove(unique(tmp$file),'.csv'),'.png'), width = 600)
      print(my_plot)
      dev.off()
   }
...
{
   write.xlsx(all, file = paste0(plot_name,"/20_20_PROJECTION.xlsx"), append = FALSE)
...

{
   tmp <- all %>%
      ...
      filter(sousplot == sous_plot_name)
   ...
   png(paste0(plot_name,"/distance_", plot_name,'.png'), width = 600)
   ...
   write.xlsx(tmp_dist, file = paste0(plot_name,"/distance.xlsx"), append = FALSE)
